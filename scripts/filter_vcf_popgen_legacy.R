...
load(paste0("../pedigree_reconstruction/ELF_pedigree_results_", "04132024", ".Robj"), verbose = T)
...
ELF_ped_inds <- ELF_ped_inds[grep("ELF", ELF_ped_inds)] # 782


PCC_ped_inds <- PCC_results[[4]]$Pedigree$id
PCC_ped_inds <- PCC_ped_inds[grep("PCC", PCC_ped_inds)] # 261
...
# PWP
#### PCC 
PCC_freqs <- t_gt_flt[grep("PCC", rownames(t_gt_flt)),]
...
#### ELF
ELF_freqs <- t_gt_flt[grep("ELF", rownames(t_gt_flt)),]
...
bb_pca <- doPCA(t_gt_flt_maf, nPCs = 4)
...
pca <- list(ELF_pca, PCC_pca)
save(pca, file = "../vcf_filtering/PCA_loadings_04132024.RObj")
